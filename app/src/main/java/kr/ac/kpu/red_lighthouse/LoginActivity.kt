...
class LoginActivity : Activity(){
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val createIdBtn : Button = binding.createId
        // 로그인 시도 이벤트
        // 아이디나 비밀번호나 공백이거나 형식에 맞지 않을 시 다시 입력하게 한다.
        // 맞을 경우 페이지 이동 로그인 정보 전송
        if(PreferenceUse.prefs.getString("id", userId.text.toString()) != ""){
            ...
            loginSubmitBtn.setOnClickListener {
                var email : String = binding.userId.text.toString()
                var password : String = binding.userPw.text.toString()
                if("".equals(email)){
